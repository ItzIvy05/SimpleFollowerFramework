...
namespace SFF_Settings {
    ...
    void Load(bool force) {
        ...
        FollowerHomes = GetPrivateProfileIntA("General", "bFollowerHomes", 1, kIniPath) != 0;
    }

    void Save() {
        auto writeInt = [](const char* key, int val) { WritePrivateProfileStringA("General", key, std::to_string(val).c_str(), kIniPath); };
        auto writeStr = [](const char* key, const char* val) { WritePrivateProfileStringA("General", key, val, kIniPath); };
